...
static int32_t sys_env_get_prior(void) {
    return curenv->env_priority;
}

static int32_t sys_env_set_prior(uint32_t prior) {
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5) {
    ...
    switch (syscallno) {
        ...
        case SYS_yield:
            sys_yield();  // No return
        case SYS_env_get_prior:
            return sys_env_get_prior();
        case SYS_env_set_prior:
            return sys_env_set_prior(a1);
        default:
            return -E_INVAL;
    }
}
